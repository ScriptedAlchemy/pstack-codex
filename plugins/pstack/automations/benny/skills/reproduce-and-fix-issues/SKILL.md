...
# Dormant operational instructions; read directly, not a registered skill.
---
...
Read `../../POLLING.md` first. It owns discovery, verified triggers, durable stage/deadline state, replay reconciliation and safe continuation across scheduled runs.
...
Prefer configured Slack app or MCP tools. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
