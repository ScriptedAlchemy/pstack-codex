...
- i want pstack installed and enabled through the supported plugin CLI on the automation host for shared dependencies.
...
Verify pstack with the supported plugin CLI and a fresh session. Do not hand-edit plugin state or write JSON into TOML. Read explicit-only dependencies directly from the installed plugin; never register dormant Benny files as plugin skills.

Read [POLLING.md](./POLLING.md) for the native scheduled-polling adapter, durable state, reconciliation and recovery. The trigger intent above maps to polling verified top-level reports, not a Slack webhook. Polling adds latency.

Commit the copied pack and secret-free configuration before enabling it, only when authorized. Configure and test tools, state storage, initial timestamp, cadence and thread safety. Do not create or update a live automation until explicitly requested. Use the supported Codex automation tool, resolving the target project first for these requested standalone project jobs. Save both schedules paused until readiness passes.

Each prompt must read POLLING.md and its exact committed operational file. Existing automations are resolved by saved ID and updated with unrelated fields preserved; never create duplicates. Follow the copied setup workflow for the full checklist.
