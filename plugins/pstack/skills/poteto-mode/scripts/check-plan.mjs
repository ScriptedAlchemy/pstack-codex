...
const PROGRAM_MARKERS = ["durable program objective", "installed pstack plugin", /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live must name ten lanes, the configured model, and the PR head`);
