...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through Codex collaboration tools. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** One per track, only when the program exceeds what one coordinator's drains can manage. Its notes are durable, not its process or agent id. A track the coordinator can drain itself needs no middle layer. Owns its track's units and boards and authors worker briefs. Delegate nested workers only when the current host permits it; otherwise the root dispatches those briefs. Never assume a fixed nesting depth or pass an environment selector. Rolls up aggregates at wave boundaries. Never forwards raw child reports. Bound the total rolling window by actual host capacity, including coordinators and reviewers, and refill as children finish.
- **Worker / verifier.** Uses the current Codex host, with explicit working-directory and permission constraints in its brief. Match transcript reads to the workspace under `~/.codex/sessions/`. Serialize access to shared browser, simulator, and local IDE state. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different configured model when available; report when only one model is available.
...
Create `<repo>/.codex/pstack-runs/<run-name>/` and set ORCH_STORE to its absolute path. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns may reference the standing-orders file by absolute store path. Paste the orders on every resume or when the receiving host cannot read that path.

A sub-coordinator brief adds its track boundary and unit list, its share of the host's available spawn budget, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher result or saved heartbeat wake (see CODEX.md), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder and checkout in the standing orders. Run restacks in that checkout, within the host's resource budget. Codex collaboration does not provision a cloud restack machine.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated verifier agent (on a different available model than the worker) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
- Never send a follow-up merely to check on an agent; that can restart an idle agent. Probe read-only through collaboration status/wait tools, the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
...
- After a Codex restart, treat old worker handles as unavailable until the host confirms otherwise. Re-read the standing orders and `units.tsv`, recompute the frontier, recover work by PR, branch, and durable report, and respawn necessary workers or sub-coordinators from stored briefs plus current state. Do not assume cloud work survived. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
