...
4. Verify each component against its baseline via image diff on the matching surface through the applicable Codex browser, computer-use, terminal, or project verification skill. A nonzero diff is a fail. Investigate the pixel delta. a Codex heartbeat automation or bounded monitor loop per component until the diff is zero.
