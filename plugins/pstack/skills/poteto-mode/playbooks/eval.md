...
5. **Spawn one blinded judge** on a different available model per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript under `~/.codex/sessions/YYYY/MM/DD/*.jsonl`, filtered to files whose first `session_meta.payload.cwd` equals the current workspace. Do not glob across `~/.codex/sessions/`. That crosses workspace boundaries and reads private chats from unrelated projects. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
