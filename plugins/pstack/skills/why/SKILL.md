...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, current selection when available, and what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available Codex app and MCP tools from the active tool map. If tool search is available, use it to discover source-specific connectors. Do not inspect unrelated configuration or assume a connector exists.
