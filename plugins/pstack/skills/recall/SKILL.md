...
Codex transcripts live under `~/.codex/sessions/YYYY/MM/DD/*.jsonl`. The first record is `session_meta`; keep only files whose `payload.cwd` equals the active workspace. Every later JSONL record is a session event. Never read another workspace's transcripts unless the user explicitly asks.
