...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `a focused diff-cleanup pass` on the diff before each commit and applies [`$unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. `a focused diff-cleanup pass` ships in the Codex-native verification tools plugin, not in pstack. If you don't have it, ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. `a focused diff-cleanup pass` cleans slop out of the code, `$unslop` cleans it out of prose, and `$no-comments` hands the comments to a reviewer who didn't write them.
